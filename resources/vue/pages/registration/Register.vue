...
<template>
    <section class="h-full gradient-form lg:h-full md:h-full w-screen  bg-gradient-to-r from-cyan-500 to-blue-500">
        <div id="registration" class="container px-6 h-auto border py-6 my-4 border-gray-200 rounded rounded-lg w-9/12 md:w-10/12 lg:w-2/4 bg-white">
...
<!--                            <input type="checkbox" value="true" id="flexCheckDefault" >-->
                            <label class="form-check-label inline-block text-gray-800">Already have a Shares and Securities</label>
            ...
            <div class="relative w-full h-full max-w-md md:h-auto flex align-items-center">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-12 h-12 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"></path></svg>
                        <input type="number" maxlength="6" :class="className.input" placeholder="OTP" v-model="auth.otp"/>
                        <button type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600" @click="resend">Resend</button>
                        <button type="button" class="text-white bg-blue-600 hover:bg-submit-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2" @click="onConfirm">
                            Verify
                        </button>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                className: {
                    ...
                    input: 'mt-1 block w-full px-3 py-2 mb-10 rounded-md border-2 border-gray-100 shadow-sm border focus:outline-none focus:ring-1 ring-indigo-500 sm:text-sm '
                },
                brokers:[
                    {name:'ZERODHA'},
                    {name:'ICICI DIRECT'},
        ...
        methods: {
            async onSubmit() {
                try {
                    let {data} = await axios.post(`/api/otp/otp-request`, this.form);
                    localStorage.setItem('form', JSON.stringify(this.form));
                    if (data) {
                        this.$toast(data.message);
                        // this.$router.push({name: 'authenticatoin'});
                        this.show = !this.show;
                    }
                } catch ({response}) {
                    this.formErrors = {};
           ...
           async onConfirm(){
                try {
                    this.auth.mobile = form.contact_no;
